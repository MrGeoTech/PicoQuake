...
#else
#define DEBUG_PRINTF( \
    ...)  // Defines DEBUG_PRINT as an empty macro when DEBUG is not defined
#endif
...
static constexpr char FIRMWARE_VERSION[] = "1.0.3";
...
// state enum
enum State { IDLE = 0, SAMPLING = 1, ERROR = 2 };

enum CommandID { HANDSHAKE = 0, START_SAMPLING = 1, STOP_SAMPLING = 2 };
...
// Any data loss (full queue, USB connection, etc) will result in
// non-incremental sample counts count is reset when starting sampling
...
// cant stop in interrupt directly, takes too long, so raise a flag and stop in
// loop
...
uint8_t out_buffer[sizeof(ImuSendStruct) +
                   4];  // start 0x00, end 0x00, cobs overhead, ID byte (right
                        // after start byte)
...
uint8_t
    out_status_buffer[Status_size + 4];  // start 0x00, end 0x00, cobs overhead,
                                         // ID byte (right after start byte)
...
ICM42688P::OutputDataRate IdxToRate(uint8_t idx) {
  switch (idx) {
    ...
    default:
      ...
      return ICM42688P::OutputDataRate::RATE_12_5;  // defaults to lowest if
                                                    // invalid
  }
}
...
ICM42688P::AccelFullScale IdxToAccelRange(uint8_t idx) {
  switch (idx) {
    ...
    default:
      ...
      return ICM42688P::AccelFullScale::RANGE_16G;  // defaults to highest if
                                                    // invalid
  }
}
...
ICM42688P::GyroFullScale IdxToGyroRange(uint8_t idx) {
  switch (idx) {
    ...
    default:
      ...
      return ICM42688P::GyroFullScale::RANGE_2000DPS;  // defaults to highest if
                                                       // invalid
  }
}
...
// start sampling at specified settings. To change settings, stop sampling and
// use this function again 0 means continuous sampling, otherwise number of
// samples to take
void StartSampling(ICM42688P::OutputDataRate rate,
                   ICM42688P::AccelFullScale accel_range,
                   ICM42688P::GyroFullScale gyro_range, FilterConfig filter_cfg,
...
void SendHandshake() {
  ...
  strncpy((char*)pb_device_info.unique_id, (char*)board_unique_id,
          sizeof(pb_device_info.unique_id));
  strncpy((char*)pb_device_info.firmware, FIRMWARE_VERSION,
          sizeof(pb_device_info.firmware) - 1);
  ...
  cobs_encode_result cobs_result =
      cobs_encode(out_buffer + 2, sizeof(out_buffer) - 3, pb_buffer, pb_size);
...
void HandleCommand(Command cmd) {
  switch (cmd.id) {
    ...
    case CommandID::START_SAMPLING:
      DEBUG_PRINTF(
          "START_SAMPLING received: rate=%d, acc=%d, gyro=%d, filter=%d\r\n",
          cmd.data_rate, cmd.acc_range, cmd.gyro_range, cmd.filter_config);
...
void SendStatus() {
  ...
  pb_status_data.missed_samples = interrupt_count;

  // nanopb
  nanopb_status_stream = pb_ostream_from_buffer(nanopb_status_buffer,
                                                sizeof(nanopb_status_buffer));
  pb_encode(&nanopb_status_stream, Status_fields, &pb_status_data);
  size_t nanopb_size = nanopb_status_stream.bytes_written;

  // COBS
  cobs_encode_result cobs_result =
      cobs_encode(out_status_buffer + 2, sizeof(out_status_buffer) - 3,
                  nanopb_status_buffer, nanopb_size);
...
void SendIMUData() {
  ...
  // COBS
  cobs_encode_result cobs_result = cobs_encode(
      out_buffer + 2, sizeof(out_buffer) - 3, &DataToSend, sizeof(DataToSend));
...
void ParseIncoming() {
  ...
  while (Serial.available()) {
    ...
    if (byte == 0x00 && buf_index == 1) {
      // two 0x00 in a row. first was incorrectly interpreted as start byte but
      // was actually end byte. This byte is now start, reset index to 1 tldr:
      // if multiple 0x00 in a row, last one is taken as start byte
      buf_index = 0;
      return;
    }
    if (byte == 0x00 && buf_index > 1) {  // end byte 0x00
      ...
      cobs_decode_result result =
          cobs_decode(incoming_cobs_buffer, sizeof(incoming_cobs_buffer),
                      incoming_buffer + 2, buf_index - 2);
      if (result.status == COBS_DECODE_OK) {
        // Use nanopb to decode the message
        pb_istream_t stream =
            pb_istream_from_buffer(incoming_cobs_buffer, result.out_len);
        // switch to decode different packet types (not needed for now, only
        // command is received)
        switch (packet_id) {
...
void setup1() { delay(6000); }

void loop() {
  // nothing to do
}

// all time consuming code is in loop1. loop1 is executed on core 1, which is
// free, core0 is busy with arduino stuff, USB comms, etc. interrupt is also on
// core1 (sampling start which enables interrupt is called inside ParseIncoming
// - interrupts run from the core they are enabled on)
void loop1() {
